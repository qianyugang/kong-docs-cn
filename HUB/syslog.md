...
| `config.successful_severity` <br> *optional*  |`info` | 分配给所有成功请求的可选日志记录严重性，响应状态代码小于400。 | 
| `config.client_errors_severity` <br> *optional* | `info` | 可选的日志记录严重性，分配给所有响应状态代码为400或更高但小于500的失败请求。。 |
| `config.server_errors_severity` <br> *optional* | `info` | 可选的日志严重性分配给所有失败的请求与响应状态代码500或更高。|
| `config.log_level` <br> *optional* | `info` | 可选的日志记录严重性，具有相同或更高严重性的任何请求都将记录到系统日志中。。 |

## 日志格式

每个请求都将以SYSLOG标准记录到系统日志中，消息组件的格式如下：
...
确保Syslog守护程序正在实例上运行，并且其日志记录级别的严重性等于或低于设置的`config.log_level`。
